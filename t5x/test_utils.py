...
def make_devices(
    ...
    hnx, hny, hnz, hnc = jax.tree.map(lambda a, b: a // b, device_bounds, host_layout)
    for x, y, z, c in itertools.product(*map(range, device_bounds)):
        hx, hy, hz, hc = jax.tree.map(lambda a, b: a // b, (x, y, z, c), host_layout)
        # TODO(levskaya, jekbradbury): verify this id/host ordering on TPU v4
        device_id = coords_to_idx(
            (c, x, y, z), (nc, nx, ny, nz)
        )  # pytype: disable=wrong-arg-types
        process_index = coords_to_idx(
            (hc, hx, hy, hz), (hnc, hnx, hny, hnz)
        )  # pytype: disable=wrong-arg-types
        devices.append(
            TpuDevice(
                id=device_id,
...
def assert_equal(a, b):
    ...
    assert isinstance(a, type(b)), f"Found incompatible types: {type(a)}, {type(b)}"
...
def get_train_state_from_variables(variables, optimizer_def=adafactor.Adafactor(0.0)):
