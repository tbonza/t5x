...
class EncoderDecoderModelTest(parameterized.TestCase):
    ...
    def test_get_initial_variables_shapes_and_types(self, shapes, types):
        ...
        with mock.patch.object(models.EncoderDecoderModel, "__init__", new=mock_init):
    ...
    def test_predict_batch_rng(self):
        ...
        decode_fn_mock = mock.Mock(return_value=(np.zeros((2, 2, 3)), np.zeros((2, 2))))
    ...
    def test_predict_batch_fake_input_shapes_and_types(self, batch):
        ...
        np.testing.assert_allclose(call_kwargs.pop("encoder_input_tokens"), fake_inputs)
        np.testing.assert_allclose(call_kwargs.pop("decoder_input_tokens"), fake_target)
        np.testing.assert_allclose(
...
class DecoderOnlyModelTest(parameterized.TestCase):
    def test_predict_batch_visible_in_prefill(self):
        ...
        batch = {
            ...
            "decoder_causal_attention": (lengths > np.arange(seq_len)).astype(np.int32),
        }
