...
Dataset = Union[tf.data.Iterator, clu.data.dataset_iterator.DatasetIterator, None]
...
def _maybe_update_ts_from_file_to_gcs(ckpt_contents):
    ...
    return jax.tree_util.tree_map(_gfile_to_gcs_driver, ckpt_contents, is_leaf=_is_leaf)
...
def _maybe_update_ts_from_gcs_to_file(ckpt_contents):
    ...
    return jax.tree_util.tree_map(_gcs_to_file_driver, ckpt_contents, is_leaf=_is_leaf)


def _get_spec(directory: str, arr: Any, name: str, metadata: Dict[str, Any]) -> ts.Spec:
...
class Checkpointer(object):
    ...
    def __init__(  # pytype: disable=annotation-type-mismatch  # jnp-type
        ...
        *,
        ...
        elif isinstance(dataset_iterator, clu.data.dataset_iterator.TfDatasetIterator):
    ...
    def _write_state_to_tensorstore(
        ...
        state_dict_for_save["target"] = (
            jax.tree_util.tree_map(  # pytype: disable=unsupported-operands  # dynamic-method-lookup
    ...
    def restore(
        ...
        if self._dataset_iterator:
            ...
            self._dataset_iterator.load(os.path.join(ckpt_dir, self._dataset_ckpt_name))
...
def _should_apply_transform_fns(ckpt_contents: PyTree, use_orbax_format: bool) -> bool:
    if use_orbax_format:
        return True
    try:
        version = ckpt_contents.get("version", 0)
...
def find_checkpoint(path: epath.PathLike, step: Optional[int] = None) -> epath.PathLike:
...
def _construct_save_args(param_info: _OrbaxParamInfo, dtype: jnp.dtype) -> ocp.SaveArgs:
...
def _construct_orbax_restoration_transforms(
    ...
    restore_args = state_utils.intersect_state(restore_args, state_dict_to_restore)
...
class OrbaxCheckpointManagerInterface:
    ...
    class _CheckpointManagerImpl(ocp.CheckpointManager):
        ...
        def _get_old_steps_to_remove(self) -> List[int]:
            ...
            if self._track_best:
                metric_name_to_monitor = (
                    self._options.metric_name_to_monitor
    ...
    def __init__(
        ...
        self._train_state = train_state
        self._partitioner = partitioner
        if isinstance(dataset_iterator, clu.data.dataset_iterator.TfDatasetIterator):
            assert dataset_iterator._checkpoint
        self._dataset_iterator = dataset_iterator
        self._save_dtype = save_dtype
    ...
    def restore(
        ...
        param_infos = _construct_orbax_param_infos(self._train_state, self._partitioner)
