...
class MetricsManager(object):
    ...
    def write_scalar(self, key: str, val: metric_writers.interface.Scalar, step: int):
    ...
    def write_metrics_summary(
        ...
        def _summarize_and_write():
            ...
            final_metrics = jax.tree_util.tree_map(utils.get_local_data, final_metrics)
...
class BaseTrainer(abc.ABC):
    ...
    def __init__(
        ...
        self.train_metrics_manager = MetricsManager("train", summary_dir=summary_dir)
    ...
    def train(
        ...
        with self._train_state_mutex:
            ...
            start_step = int(start_step if start_step is not None else train_state.step)
    ...
    def eval(
        ...
        for iter_name, batch_iter in batch_iters.items():
            ...
            eval_summaries[iter_name] = (
                mm.write_metrics_summary(  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    def compile_eval(self, batches: Mapping[str, BatchType]) -> None:
        ...
        for eval_name, batch in batches.items():
            ...
            tock = _time()
            self.eval_metrics_managers[
                eval_name
            ].write_scalar(  # pytype: disable=wrong-arg-types  # jax-ndarray
...
) -> Tuple[train_state_lib.TrainState, MutableMetricMapType, Optional[FlaxMutables]]:
...
def eval_step(
    ...
    if not train_state.flax_mutables:
        ...
        _, metrics = model.eval_fn(train_state.params, batch, train_state.flax_mutables)
    ...
    def _partitioned_train_step(self) -> PartitionedTrainCallable:
        def train_step(train_state: train_state_lib.TrainState, batch: BatchType):
            return train_with_lr(
                ...
                dropout_rng=self._get_step_rng(
                    train_state.step
                ),  # pytype: disable=wrong-arg-types  # jax-ndarray
                model=self._model,
