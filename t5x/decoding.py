        ...
        def sample_logits_with_nonzero_temperature(logits, temperature):
            ...
            if _is_tracer(topk) or topk:
                scaled_logits = jax.lax.cond(
                    topk > 0,
                    lambda: binary_search.topk_mask(
...
def beam_search(
    ...
    def beam_search_loop_body_fn(state: BeamState) -> BeamState:
        ...
        new_scores = topk_log_probs / brevity_penalty(
            alpha, lengths
        )  # pytype: disable=wrong-arg-types  # jax-devicearray
    ...
    if right_aligned_input is not None:
        ...
        # --> [batch, beams, length]
        finished_seqs = jnp.concatenate([right_aligned_input, finished_seqs], axis=-1)

        # Now we left align everything.

        # First flatten to [batch_size * beams, length]
        flat_finished_seqs = jnp.reshape(finished_seqs, (-1, finished_seqs.shape[-1]))
