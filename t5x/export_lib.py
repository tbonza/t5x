...
PostprocessorFn = Callable[[Tuple[Any, Any]], Union[Tuple[Any, Any], Mapping[str, Any]]]
...
def flatten(compute_outputs: PyTree, assert_output_len=None) -> Tuple[jnp.ndarray, ...]:
...
def create_inference_function(
    *,
    ...
    if inference_mode.model_fn_const_kwargs:
        model_fn = functools.partial(model_fn, **inference_mode.model_fn_const_kwargs)
...
def load_params_from_checkpoint(
    ...
    if train_state_initializer is not None:
        ...
        variables = checkpoints.load_t5x_checkpoint(
            ...
            state_transformation_fns=(restore_checkpoint_cfg.state_transformation_fns),
...
def truncate_and_pad_tokenized_input(
    ...
    input_tensor = tf.pad(input_tensor, [[0, max_length - tf.shape(input_tensor)[0]]])
...
def create_decoder_preprocessor(
    ...
    def preprocess(input_texts: tf.Tensor) -> Mapping[str, tf.Tensor]:
        ...
        if input_feature == "inputs":
            ...
            ragged_input_tokens = tf.RaggedTensor.from_tensor(decoder_tokens, padding=0)
        ...
        positions = tf.repeat([positions], tf.shape(decoder_target_tokens)[0], axis=0)
...
def _request_for_batch(
    ...
    if batch_size is not None:
        adjusted_batch = list(itertools.islice(itertools.cycle(text_batch), batch_size))
...
def create_batch_polymorphic_shapes(
    ...
    *,
    ...
    is_poly_batch = not all(
        s.shape.is_fully_defined() for s in jax.tree_util.tree_leaves(input_signature)
    )
...
def save(
    *,
    ...
    partitioner: Optional[partitioning.BasePartitioner],
    create_decoding_state_callback_fn: Optional[CreateDecodingStateCallbackFn] = None,
    output_features: Optional[Mapping[str, seqio.Feature]],
    task_feature_lengths: Mapping[str, int],
    batch_size: Optional[int],
