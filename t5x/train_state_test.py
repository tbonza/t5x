...
class FlaxOptimTrainStateTest(absltest.TestCase):
    def test_init(self):
        ...
        self.assertEqual(
            state.state_dict()["flax_mutables"], flax.core.unfreeze(flax_mutables)
        )
        jax.tree.map(np.testing.assert_array_equal, params, state.params)
        jax.tree.map(
    ...
    def test_create(self):
        ...
        state = train_state_lib.FlaxOptimTrainState.create(
            optimizer_def, model_variables
        )
        self.assertEqual(state.step, 0)
        self.assertIsInstance(state._optimizer, optimizers.Optimizer)
        ...
        jax.tree.map(
            np.testing.assert_array_equal, state.params, model_variables["params"]
        )
    ...
    def test_create_with_params_axes(self):
        ...
        jax.tree.map(
            np.testing.assert_array_equal, model_variables["params"], state.params
        )
        jax.tree.map(
            np.testing.assert_array_equal,
    ...
    def test_create_missing_params_axes(self):
        ...
        with self.assertRaisesWithLiteralMatch(
            ...
            train_state_lib.FlaxOptimTrainState.create(
                adafactor.Adafactor(), model_variables
            )
    ...
    def test_replace_params(self):
        ...
        jax.tree.map(np.testing.assert_array_equal, new_params, new_state.params)
    ...
    def test_apply_gradient(self):
        ...
        optimizer = mock.Mock(apply_gradient=mock.Mock(return_value=updated_optimizer))
        state = train_state_lib.FlaxOptimTrainState(optimizer)
    ...
    def test_to_state_dict(self):
        ...
        jax.tree.map(
            ...
            state.state_dict(),
            {
                ...
                "target": {"kernel": np.zeros((2, 4))},
                "flax_mutables": {"mutables": np.ones(3)},
            },
        )
    ...
    def test_restore_state(self):
        ...
        restored = state.restore_state(
            {
                ...
                "target": {"kernel": np.ones((2, 4))},
                "flax_mutables": {"mutables": np.zeros(3)},
            }
        )
        ...
        jax.tree.map(
            np.testing.assert_array_equal,
            restored.param_states,
            flax.core.freeze(
                {
                    "kernel": adafactor._AdafactorParamState(
...
class InferenceStateTest(absltest.TestCase):
    ...
    def test_restore_state(self):
        ...
        self.assertEqual(
            restored.params_axes, {"bias_axes": AxisMetadata(names=("embed",))}
        )
