...
def _scale_by_schedule_ctor(state, params_axes):
    ...
    return (
        optax.ScaleByScheduleState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=None
        )
    )
...
class OptaxStatePartitionRules:
    ...
    _RULES = {
        ...
        optax.AddNoiseState: lambda state,
        params_axes: optax.AddNoiseState(  # pytype: disable=wrong-arg-types  # numpy-scalars
        ...
        optax.EmaState: lambda state,
        params_axes: optax.EmaState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=None,
            ema=OptaxStatePartitionRules.derive_params_axes(state.ema, params_axes),
        ),
        optax.EmptyState: lambda state, params_axes: optax.EmptyState(),
        optax.TraceState: lambda state, params_axes: optax.TraceState(
            trace=OptaxStatePartitionRules.derive_params_axes(state.trace, params_axes)
        ),
        optax.ScaleByAdamState: lambda state,
        params_axes: optax.ScaleByAdamState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=None,
        ...
        optax.ScaleByBeliefState: lambda state,
        params_axes: optax.ScaleByBeliefState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=None,
            mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes),
            nu=OptaxStatePartitionRules.derive_params_axes(state.nu, params_axes),
        ),
        optax.ScaleByLionState: lambda state,
        params_axes: optax.ScaleByLionState(  # pytype: disable=wrong-arg-types  # numpy-scalars
            count=None,
            mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes),
        ),
        ...
        optax.InjectHyperparamsState: lambda state,
        params_axes: optax.InjectHyperparamsState(  # pytype: disable=wrong-arg-types  # jax-ndarray
            count=None,
        ...
        optax.MultiStepsState: lambda state,
        params_axes: optax.MultiStepsState(  # pytype: disable=wrong-arg-types  # jax-ndarray
            mini_step=None,
            gradient_step=None,
        ...
        optax.ConditionallyTransformState: lambda state,
        params_axes: optax.ConditionallyTransformState(  # pytype: disable=wrong-arg-types  # jax-ndarray
            inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(
                state.inner_state, params_axes
            ),
            step=None,
...
class OptaxWrapper(OptimizerDef):
    ...
    def derive_logical_axes(self, optimizer, param_logical_axes):
        ...
        """
        optimizer_logical_axes = jax.tree.map(lambda x: None, optimizer.state_dict())
...
class MultiOptimizer(OptimizerDef):
    ...
    def derive_logical_axes(self, optimizer, param_logical_axes):
        ...
        param_states = jax.tree.map(lambda x: _Marker(), optimizer.state.param_states)
        for focus, opt_def in zip(self.traversals, self.sub_optimizers):
            if hasattr(opt_def, "derive_logical_axes"):
                ...
                new_opt = opt_def.derive_logical_axes(
                    Optimizer(opt_def, OptimizerState(None, ss), ps),
                    ps,  # pytype: disable=wrong-arg-types  # jax-ndarray
                )
                param_states = _update_subtree_of_traversal(
