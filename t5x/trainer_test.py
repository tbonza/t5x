...
class MetricsManagerTest(absltest.TestCase):
    ...
    def test_write_metrics_summary(self):
        ...
        with (
            ...
            summary = mm.write_metrics_summary(accumulated_metrics, step=4, num_steps=2)
            mm.flush()
...
class TrainerTest(parameterized.TestCase):
    def setUp(self):
        ...
        self.init_train_state = train_state_lib.FlaxOptimTrainState(self.init_optimizer)
        ...
        self.dataset = tf.data.Dataset.range(6).map(mapfn).batch(2, drop_remainder=True)
    ...
    def _test_train(self, precompile):
        ...
        expected_metrics = {k: v + 2 * num_steps for k, v in initial_metrics.items()}
        # (0 + 2) / 2 = 1
        ...
        jax.tree.map(np.testing.assert_equal, trainer.train_state, expected_train_state)
    ...
    def test_early_stopping_action(self, mode, metrics, atol, rtol, stop_training):
        trainer = self.test_trainer
    ...
    def test_early_stopping_action_error(self, task, metric, value):
        ...
        trainer_stop_training = hook.run(trainer.train_state, {task: {metric: value}})
    ...
    def test_compile_eval(self):
        ...
        with mock.patch("t5x.trainer._time", side_effect=[1, 5, 6, 9, 10, 11, 12, 13]):
    ...
    def test_accumulate_grads_microbatched_without_weight_sum_single_batch(self):
        ...
        i = batch["i"].sum()
        expected_grad_accum = jax.tree.map(lambda x: i, self.init_train_state).params
...
class TrainerRngDeterminismTest(parameterized.TestCase):
    ...
    def test_rng_determinism(self, mock_accum_grads):
        ...
        np.testing.assert_array_equal(metrics.result()["rng"].value, expected_rng_sum)
...
def fake_mut_apply_grads(
    ...
    metrics["learning_rate"] = clu.metrics.Average.from_model_output(learning_rate)
    optimizer = jax.tree.map(lambda x, g: x + g, optimizer, grad_accum)
    return optimizer, metrics
...
class MutableTrainerTest(parameterized.TestCase):
    ...
    @mock.patch("jax.value_and_grad", fake_value_and_grad_fn_wo_weight_sum_w_mutables)
    def test_accumulate_grads_microbatched_without_weight_sum_multiple_batches(
        self,
    ):
        batch_iter = self.dataset.as_numpy_iterator()
