...
def train(
    *,
    ...
    def _verify_matching_vocabs(cfg: utils.DatasetConfig):
        if verify_matching_vocabs_fn and isinstance(model, models.BaseTransformerModel):
    ...
    host_step = int(
        utils.get_local_data(train_state.step)
    ...
    if set(actions.keys()).difference(_ACTION_KEYS):
        raise ValueError(
            f"actions keys must be one of {_ACTION_KEYS}, but got : {actions.keys()}"
        )
    ...
    if total_steps < first_step:
        raise ValueError(
            f"Unexpected total_steps ({total_steps}) < checkpoint step  ({first_step})."
        )
            ...
            while host_step < epoch_end_step:
                ...
                if not concurrent_metrics:
                    ...
                    trainer.stop_training = (
                        run_actions(  # pytype: disable=wrong-arg-types  # jax-ndarray
