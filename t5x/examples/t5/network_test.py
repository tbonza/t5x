...
def get_test_model(
    ...
    return models.EncoderDecoderModel(module, vocab, vocab, optimizer_def=optimizer_def)
...
class NetworkTest(parameterized.TestCase):
    def setUp(self):
        ...
        np.random.seed(42)
        self.batch = {
            "encoder_input_tokens": np.random.randint(
                3, 10, size=(batch_size, input_len)
            ),
            "decoder_input_tokens": np.random.randint(
                3, 10, size=(batch_size, max_decode_len)
            ),
    ...
    def test_t5_1_1_regression(self):
        ...
        params = model.get_initial_variables(jax.random.PRNGKey(42), self.input_shapes)[
            "params"
        ]
        loss, _ = jax.jit(model.loss_fn)(params, batch, jax.random.PRNGKey(1))  # pylint: disable=unused-variable
