...
class EncoderDecoderModel(models.EncoderDecoderModel):
    ...
    def loss_fn_meta_cls(
        ...
        for meta_logits, meta_labels in zip(all_meta_logits[:-1], all_meta_labels[:-1]):
            # Balance across the positive/ negative labels.
            balanced_weights = weights.copy().astype(
                float
    ...
    def loss_fn_meta_cls_geom_like(
        ...
        metrics = (
            self._compute_metrics(  # pytype: disable=wrong-arg-types  # jax-ndarray
                logits=all_logits[-1],
                targets=batch["decoder_target_tokens"],
                mask=weights,
                loss=loss,
