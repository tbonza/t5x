...
def temperature_sample(
    ...
    state_callback_fn: Optional[
        Callable[[SamplingLoopState], SamplingLoopState]
    ] = None,
    logit_callback_fn: Optional[
        Callable[[jnp.ndarray, SamplingLoopState], jnp.ndarray]
    ] = None,
    ...
    if (
        ...
        max_decode_steps = jnp.minimum(max_decode_steps, max_decode_steps_hard_limit)
    ...
    expanded_decodes, expanded_log_prob, expanded_exits, expanded_confidences = (
    ...
    decodes = decoding.unflatten_beam_dim(expanded_decodes, batch_size, num_decodes)
    exits = decoding.unflatten_beam_dim(expanded_exits, batch_size, num_decodes)
    confidences = decoding.unflatten_beam_dim(
        expanded_confidences, batch_size, num_decodes
    )
    # [batch * num_decodes] -> [batch, num_decodes]
    log_prob = decoding.unflatten_beam_dim(expanded_log_prob, batch_size, num_decodes)
    ...
    sorted_log_prob = jnp.take_along_axis(log_prob, jnp.squeeze(idxs, axis=-1), axis=-1)
...
def _temperature_sample_single_trial(
    ...
    def sampling_loop_body_fn(state: SamplingLoopState) -> SamplingLoopState:
        ...
        def sample_logits_with_nonzero_temperature(logits):
            ...
            if topk:
                scaled_logits = binary_search.topk_mask(
                    scaled_logits, topk, NEG_INF
            ...
            if _is_tracer(topp) or topp:
                scaled_logits = binary_search.topp_mask(
                    scaled_logits, topp, NEG_INF
                )  # pytype: disable=wrong-arg-types  # jax-ndarray
        ...
        # [batch] -> [batch]
        next_log_prob = state.log_prob + (next_log_prob * out_of_prompt) * jnp.squeeze(
            ~state.ended, axis=-1
        ).astype(jnp.int32)
        ...
        new_sequences = state.sequences * (1 - one_hot) + next_token_or_endpad * one_hot
