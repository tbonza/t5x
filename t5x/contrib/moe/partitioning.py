...
def default_moe_mesh(
    ...
    devices = base_default_mesh.devices.reshape(-1, expert_axis_size, model_axis_size)
    global_mesh = Mesh(devices, ["data", "expert", "model"])
    logging.info("Overridden MoE global_mesh axes_names: %s", global_mesh.axis_names)
    logging.info("Overridden MoE global_mesh devices: %s", global_mesh.devices)
    logging.info("Overridden MoE global_mesh shape: %s", global_mesh.devices.shape)
    return global_mesh
...
class MoePjitPartitioner(base_partitioning.PjitPartitioner):
    ...
    def get_logical_axes(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
        ...
        state_filter_fn = self._state_filter_fn or _infer_state_filter_fn(train_state)
