...
class LegacyCheckpointer(orbax.checkpoint.Checkpointer):
    ...
    def restore(
        ...
        if from_tensorflow:
            logging.info("Initializing parameters from TensorFlow checkpoint %s", path)
...
def restore(
    ...
    for path in paths:
        logging.info("Initializing parameters from specific T5X checkpoint %s", path)
...
def prepare_train_iter(
    train_iter: Union[tf.data.Dataset, clu.data.dataset_iterator.DatasetIterator],
    *,
    partitioner,
    checkpoint_cfg,
    data_layout,
...
def get_fallback_state(
    ...
    if restore_cfg.fallback_to_scratch:
        if not restore_cfg.state_transformation_fns:
            raise ValueError(
                "`state_transformation_fns` must be provided with `fallback_to_scratch`"
            )
        if init_rng is None:
            raise ValueError(
...
class TrainStateInitializer:
    ...
    * To restore from a checkpoint with a fallback to initializing from scratch,
    ...
    def __init__(
        ...
        self.train_state_axes = partitioner.get_mesh_axes(self.global_train_state_shape)
    ...
    *,
    ...
    def _get_extra_kwargs(cfg):
        ...
        if isinstance(cfg.checkpointer_cls, functools.partial):
            ...
            if issubclass(type(cfg.checkpointer_cls), checkpoints.SaveBestCheckpointer):
    ...
    def infer_fn(
        ...
        if len(indices_and_outputs) != original_ds_length:
            raise ValueError(
                ...
                "dataset: %d versus %d" % (len(indices_and_outputs), original_ds_length)
            )
...
def import_module(module: str):
    ...
    try:
        ...
        if str(e) == "Attempted to add a new configurable after the config was locked.":
            raise RuntimeError(
...
def get_vocabulary(
    ...
    if isinstance(cfg.mixture_or_task_name, airio.DatasetProviderBase):
        ...
        features = {
            k: seqio.Feature(vocabulary=v) for k, v in vocab_map.items()
        }  # pytype: disable=wrong-arg-types
...
def verify_matching_vocabs(cfg: DatasetConfig, model: models.BaseTransformerModel):
...
def get_dataset(
    ...
    return get_dataset_inner(cfg, shard_info, feature_converter_cls, seed, num_epochs)
...
def round_vocab_size_to_multiple(vocabulary: seqio.Vocabulary, divisor: int = 128):
...
def override_params_axes_names(
    ...
    model_variables["params_axes"] = traverse_util.unflatten_dict(flat_params_axes)
...
def find_next_checkpoint_step(
    ...
    while (
        checkpoint_steps
        and (inner_num_steps + host_step) >= checkpoint_steps[checkpoint_steps_index]
    ):
