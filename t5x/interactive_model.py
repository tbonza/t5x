...
BatchesType = Union[Sequence[Mapping[str, str]], Sequence[Sequence[Mapping[str, str]]]]
...
class InteractiveModel(abc.ABC):
    ...
    def __init__(
        ...
        self._checkpoint_manager = utils.LegacyCheckpointManager(
            ...
            train_state_shape=(self._train_state_initializer.global_train_state_shape),
            partitioner=self._partitioner,
            ds_iter=None,
        ...
        restore_cfgs.append(
            utils.RestoreCheckpointConfig(
                ...
                restore_dataset=(
                    self._save_checkpoint_cfg and self._save_checkpoint_cfg.save_dataset
                ),
    ...
    def train_step_from_batch_iterator(self, iterator):
        ...
        self._train_summary = self._trainer.train(iterator, 1, start_step=first_step)
    ...
    def infer_with_preprocessors(
        ...
        **inference_kwargs,
        ...
        if mode == InferenceType.PREDICT_WITH_AUX:
            ...
            raise ValueError(
                f"Mode must be `predict_with_aux`, or `score`, but instead was {mode}."
            )
    ...
    def score(self, examples: Sequence[Union[str, dict[str, str]]]) -> Sequence[Any]:
        """Score method."""
        # By default, only tokenize and append EOS.
        preprocessors = [
            seqio.preprocessors.tokenize,
    ...
    def _compute_metrics(
        ...
        predict_with_aux_metric_fns: Sequence[seqio.dataset_providers.MetricFnCallable],
            ...
            if step_num % eval_period == 0:
                ...
                if predict_batches:
                    for predict_batch in predict_batches:
                        predictions, _ = self.predict_with_aux(
                            predict_batch
...
def _get_equal_length_batches(batches: BatchesType, length: int) -> Sequence[Any]:
...
def get_seqio_task_from_examples(
    ...
    examples: Sequence[Union[str, dict[str, str]]],
    preprocessors: Sequence[Callable[..., tf.data.Dataset]],
    metric_fns: Optional[Sequence[seqio.dataset_providers.MetricFnCallable]] = None,
