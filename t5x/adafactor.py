...
class Adafactor(OptimizerDef):
    ...
    def _parse_rule(
        ...
        batched_dims = tuple(idx for idx, d in enumerate(rule) if d == FactorDim.BATCH)
    ...
    def init_state(self, params):
        ...
        param_states_flat = [
            self.init_param_state(param, path) for path, param in params_flat.items()
        ]
        param_states_flat = {
    ...
    def apply_param_gradient(self, step, hyper_params, param, state, grad, path):
        ...
        if self.hyper_params.factored:
            ...
            factor_rule = None
        averaging_dims, factored_dims = self._parse_rule(factor_rule, param.shape, path)
