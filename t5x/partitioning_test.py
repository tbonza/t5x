...
class PartitioningTest(absltest.TestCase):
    ...
    def test_default_mesh(self, process_index_fn, devices_fn, local_devices_fn):
        ...
        local_devices_fn.return_value = [d for d in TPUV3_32 if d.process_index == 0]
...
class ModelBasedPartitionerTest(parameterized.TestCase):
    ...
    def test_get_mesh_axes(self):
        ...
        expected_axes_spec = self.get_expected_axes_spec(
            adafactor._AdafactorParamState(m=p0_spec, v=None, v_col=None, v_row=None),
            adafactor._AdafactorParamState(m=p1_spec, v=None, v_col=None, v_row=None),
        )
        jax.tree.map(self.assertEqual, axes_spec, expected_axes_spec)
        ...
        expected_axes_spec = self.get_expected_axes_spec(
            adafactor._AdafactorParamState(m=None, v=p0_spec, v_col=None, v_row=None),
            adafactor._AdafactorParamState(m=None, v=p1_spec, v_col=None, v_row=None),
        )
        jax.tree.map(self.assertEqual, axes_spec, expected_axes_spec)
