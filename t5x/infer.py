...
def infer(
    *,
    ...
    task_or_mixture = seqio.maybe_get_mixture_or_task(dataset_cfg.mixture_or_task_name)
    ...
    input_shapes = {k: (batch_size,) + spec.shape for k, spec in element_spec.items()}
    input_types = {
        k: jnp.dtype(spec.dtype.as_numpy_dtype) for k, spec in element_spec.items()
    }
