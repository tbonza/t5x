...
def run(main):
    ...
    utils.run_main(
        main,
        flags_parser=lambda a: app.parse_flags_with_usage(list(rewrite_gin_args(a))),
...
@gin.configurable
def string_split_fn(text=gin.REQUIRED, separator=gin.REQUIRED, maxsplit=-1, index=None):
